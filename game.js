...
(function (global) {
  ...
  var runFrames = 0;       // кадров с начала забега — из них считается fps
  var runPlaySec = 0;      // и сколько секунд игра при этом реально шла
  var newBest = false;
  ...
  function resetRun() {
    ...
    runFrames = 0;
    runPlaySec = 0;
    // Пересборку канваса держим до конца забега: баннер может выехать в
    // любой момент, а перестраивать буфер кадра под руками у игрока нельзя.
  ...
  function die() {
    ...
    adOffer = '';
    shake = 1;
    // Забег кончился — теперь можно и канвас пересобрать, и баннер попросить.
    core.releaseResize();
    showBanner('dead');
    S.death();
  ...
  function update(dt, time) {
    ...
    /* Кадры забега считаем здесь: жалобы на «лагает» надо чем-то мерить,
       иначе остаётся верить на слово и чинить наугад. Время копим своё, а не
       по часам: пока крутится ролик, кадров нет, и настенное время посчитало
       бы продолженные забеги медленными на ровном месте — то есть испортило
       бы ровно то сравнение, ради которого всё и затевается. */
    if (state === 'play') {
      runFrames++;
      runPlaySec += dt;
    }
  ...
  function revive() {
    ...
    camY = ball.y - 150;
    ensureStars();
    // Забег продолжается — значит и придержать пересборку надо снова.
    core.holdResize = true;
    state = 'play';
    P.gameplayStart();
  }
  ...
  /* Баннер поднимается один раз и дальше висит всегда, забег ему не помеха.

     Прятать его на время забега пробовали и отказались: сессия идёт 134
     секунды, из них 77 внутри забегов, то есть пряча баннер мы выбрасываем
     больше половины показов у формата, заведённого ровно ради показов.

     Больно не от того, что баннер висит, а от того, что он возникает
     посреди забега: окно укорачивается, и канвас пересобирается под руками
     у игрока. Лечится это не прятанием, а отложенной пересборкой —
     core.holdResize, — поэтому появление баннера в забеге теперь ничего не
     стоит, а разбирается всё на ближайшем экране смерти.
