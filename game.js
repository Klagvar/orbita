...
(function (global) {
  ...
  /* Баннер держим только между забегами.
     ...
     Сначала он поднимался один раз на загрузке и висел всегда — рассуждение
     было такое, что одна перестройка канваса дешевле многих. Живьём вышло
     иначе: ВК показывает баннер не когда его попросили, а когда у него
     появится материал, и на телефоне это случилось через пятнадцать секунд
     после запуска, то есть посреди забега. Окно в этот момент меняет размер,
     канвас пересобирается, игра спотыкается — а игрок в этот момент летит
     между звёздами.

     Момент показа мы не контролируем, зато контролируем, когда баннера точно
     быть не должно. Поэтому перед забегом прячем, после смерти показываем:
     перестройка попадает на экран, где она никому не мешает. Показов это
     стоит, но неиграбельная игра стоит дороже. */
