...
(function (global) {
  ...
  function startRun() {
    S.click();
    // Прячем до забега: пересборка канваса должна случиться сейчас, а не
    // через десять секунд, когда игрок будет между звёздами.
    P.hideBanner();
    runs++;
  ...
  function revive() {
    ...
    camY = ball.y - 150;
    ensureStars();
    /* Здесь это обязательно. Баннер запрашивается на экране смерти, а ответ
       от ВК приходит секунд через десять — то есть ровно тогда, когда игрок
       уже досмотрел ролик и снова летит. Именно так баннер и приземлялся
       посреди забега. */
    P.hideBanner();
    state = 'play';
    P.gameplayStart();
  ...
  /* Баннер держим только между забегами.

     Обе стороны проверены живьём. Висящий в забеге баннер стоит нам заметной
     части кадров: телефон рисует нашу игру и чужую рекламу одновременно и не
     тянет. Спрятанный — стоит показов: сессия идёт 134 секунды, из них 77
     внутри забегов, то есть больше половины времени формата не будет видно.

     Выбран второй вариант. Тормозящая игра портит удержание, а удержание —
     это и есть то, что мы сейчас меряем; испорченный замер обойдётся дороже
     половины показов у одной игры.

     Просим повторно на экране смерти: перестройка окна попадает туда, где
