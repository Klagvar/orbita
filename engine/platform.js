...
(function (global) {
  ...
  /* Сколько ждём ответа от моста, прежде чем считать показ несостоявшимся.
     Было десять секунд — и по телеметрии видно, чем это кончается: игрок
     жмёт «продолжить», ролик с наградой не отвечает вообще, и десять
     секунд неподвижного экрана он читает как поломку. Обе такие сессии
     закончились выходом из игры ровно на этом месте.

     Три с половиной секунды — это примерно та задержка, которую ещё
     терпят, приняв за загрузку. Настоящая реклама успевает: межэкранная
     на том же телефоне отвечает за полторы. */
  var AD_TIMEOUT = 3500;
  ...
  Platform.prototype._showAd = function (format, waterfall) {
    ...
    return new Promise(function (resolve) {
      ...
      var timer = setTimeout(function () {
        ...
        /* Молчание не разовое: если ролик с наградой не ответил раз, он не
           отвечает и дальше. Убираем кнопку до конца сессии, чтобы игрок не
           упирался в неё снова и снова. Проверка инвентаря тут не помощник —
           она бодро отвечает «ролик есть» и в этом случае тоже. */
        if (format === 'reward') self.rewardedReady = false;
        finish(false);
      }, AD_TIMEOUT);
